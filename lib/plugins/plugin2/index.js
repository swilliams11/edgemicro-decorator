  ...
  init: function(config, logger, stats) {
    return {
     ...
     ondata_request: function(req, res, data, next) {
       debug('plugin-2: ondata_request');
       next(null, data);
     },

     onend_request: function(req, res, data, next) {
       debug('plugin-2: onend_request');
       next(null, data);
     },
     ...
     onerror_request: function(req, res, data, next) {
       debug('plugin-2: onerror_request');
       next(null, data);
     },

     //RESPONSE
     onresponse: function(req, res, data, next) {
       debug('plugin-2: onresponse ' + data.length);
       next(null, data);
     },

     ondata_response: function(req, res, data, next) {
       debug('plugin-2: ondata_response ' + data.length);
       next(null, data);
     },

     onend_response: function(req, res, data, next) {
       debug('plugin-2: onend_response');
       next(null, data);
     }
     ...
     onerror_response: function(req, res, data, next) {
       debug('plugin-2: onerror_response');
       next(null, data);
     }
    };
  }
