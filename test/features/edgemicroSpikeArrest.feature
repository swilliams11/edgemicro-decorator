Feature:
    Make sure that the spike arrest works correctly. By default EM starts with
    ...
    Scenario: Send third valid request to Cloud Foundry which returns hello world
      Given I set Authorization header to Bearer `accessToken`
      When I GET /edgemicro_hello/greeting
      Then response body should be valid json
      And response code should be 503
      And response body should contain SpikeArrest
